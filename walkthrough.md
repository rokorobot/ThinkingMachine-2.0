...
The system includes a complete pipeline for self-improvement via LoRA fine-tuning.

### How It Works
1. **Distillation**: The system mines high-quality traces (reward > 0.7) to create a training dataset.
2. **Training**: The `train_tm_model.py` worker loads a base model, applies LoRA adapters, and fine-tunes it on the dataset.
3. **Versioning**: The new model is registered in the `model_versions` table as a "candidate".
4. **Evaluation**: The Orchestrator compares the candidate against the baseline in a tournament.

### Triggering Training
You can trigger the training loop via the Admin API:
...
Or run the worker manually for a specific run ID:
```bash
